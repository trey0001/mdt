InfinityFW = exports['infw-core']:GetCoreObject()
local PlayerData = {}
local CurrentCops = 0
local isOpen = false
local callSign = ""
...
-- Events from InfinityFW
RegisterNetEvent('InfinityFW:Client:OnPlayerLoaded', function()
    PlayerData = InfinityFW.Functions.GetPlayerData()
    callSign = PlayerData.metadata.callsign
end)

RegisterNetEvent('InfinityFW:Client:OnPlayerUnload', function()
    TriggerServerEvent("ps-mdt:server:OnPlayerUnload")
    PlayerData = {}
end)

RegisterNetEvent('InfinityFW:Client:OnJobUpdate', function(JobInfo)
    PlayerData.job = JobInfo
end)

RegisterNetEvent('InfinityFW:Client:OnGangUpdate', function(GangInfo)
    PlayerData.gang = GangInfo
end)

RegisterNetEvent("InfinityFW:Client:SetDuty", function(job, state)
...
RegisterNetEvent('InfinityFW:Player:SetPlayerData', function(val)
    PlayerData = val
end)
...
RegisterCommand('mdt', function()
    ...
    if not PlayerData.metadata["isdead"] and not PlayerData.metadata["inlaststand"] and not PlayerData.metadata["ishandcuffed"] and not IsPauseMenuActive() then
        ...
        InfinityFW.Functions.Notify("Can't do that!", "error")
    end
end, false)
...
RegisterNetEvent('mdt:client:dashboardWarrants', function()
    InfinityFW.Functions.TriggerCallback("mdt:server:getWarrants", function(data)
        if data then
...
RegisterNUICallback("searchProfiles", function(data, cb)
    ...
    InfinityFW.Functions.TriggerCallback('mdt:server:SearchProfile', function(result)
        p:resolve(result)
    end, data.name)
...
RegisterNUICallback("getProfileData", function(data, cb)
    ...
    local getProfileDataPromise = function(data)
        ...
        p = promise.new()
        InfinityFW.Functions.TriggerCallback('mdt:server:GetProfileData', function(result)
            p:resolve(result)
        end, data)
    ...
    --[[ local getProfileProperties = function(data)
        ...
        pP = promise.new()
        InfinityFW.Functions.TriggerCallback('infw-phone:server:MeosGetPlayerHouses', function(result)
            pP:resolve(result)
        end, data)
...
RegisterNUICallback('SetHouseLocation', function(data, cb)
    ...
    InfinityFW.Functions.Notify('GPS has been set!', 'success')
end)
...
RegisterNUICallback("searchVehicles", function(data, cb)
    ...
    InfinityFW.Functions.TriggerCallback('mdt:server:SearchVehicles', function(result)
        p:resolve(result)
    end, data.name)
...
RegisterNUICallback("saveVehicleInfo", function(data, cb)
    ...
    if JobType == 'police' and impound.impoundChanged == true then
        if impound.impoundActive then
            ...
            if found == 0 then
                InfinityFW.Functions.Notify('Vehicle not found!', 'error')
...
RegisterNUICallback("toggleDuty", function(data, cb)
    TriggerServerEvent('InfinityFW:ToggleDuty')
    cb(true)
end)
...
RegisterNetEvent('mdt:client:setRadio', function(radio)
    if type(tonumber(radio)) == "number" then
        ...
        InfinityFW.Functions.Notify("You have set your radio frequency to "..radio..".", "success")
    else
        InfinityFW.Functions.Notify("Invalid Station(Please enter a number)", "error")
    end
end)
