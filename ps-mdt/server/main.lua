local InfinityFW = exports['infw-core']:GetCoreObject()
-- Maybe cache?
local incidents = {}
local convictions = {}
local bolos = {}
...
if Config.UseWolfknightRadar == true then
	...
	AddEventHandler("wk:onPlateScanned", function(cam, plate, index)
		local src = source
		local Player = InfinityFW.Functions.GetPlayer(src)
...
AddEventHandler("playerDropped", function(reason)
	...
	if player ~= nil then
		...
		local license = InfinityFW.Functions.GetIdentifier(src, "license")
...
InfinityFW.Functions.CreateCallback('mdt:server:SearchProfile', function(source, cb, sentData)
...
InfinityFW.Functions.CreateCallback("mdt:server:getWarrants", function(source, cb)
    local WarrantData = {}
...
InfinityFW.Functions.CreateCallback('mdt:server:OpenDashboard', function(source, cb)
...
InfinityFW.Functions.CreateCallback('mdt:server:GetProfileData', function(source, cb, sentId)
...
InfinityFW.Functions.CreateCallback('mdt:server:SearchVehicles', function(source, cb, sentData)
	if not sentData then  return cb({}) end
	local src = source
	local PlayerData = GetPlayerData(src)
...
RegisterNetEvent('mdt:server:setCallsign', function(cid, newcallsign)
	local Player = InfinityFW.Functions.GetPlayerByCitizenId(cid)
...
RegisterNetEvent('mdt:server:setWaypoint', function(callid)
	local src = source
	local Player = InfinityFW.Functions.GetPlayer(source)
	local JobType = GetJobType(Player.PlayerData.job.name)
	if JobType == 'police' or JobType == 'ambulance' then
		if callid then
...
RegisterNetEvent('mdt:server:setRadio', function(cid, newRadio)
	...
	if Player.PlayerData.citizenid ~= cid then
		TriggerClientEvent("InfinityFW:Notify", src, 'You can only change your radio!', 'error')
		return
	else
		...
		if radio ~= nil then
			...
			TriggerClientEvent("InfinityFW:Notify", src, 'You do not have a radio!', 'error')
		end
	end
